...
makeCacheMatrix <- function(x=matrix()){
    ...
    ## Method to set the matrix from website
    set <- function(y){
        matrix <<- y
        inv <<- NULL
    }
...
##  then the "cacheRetri" should retrieve the inverse from the cache.

cacheRetri <- function(x, ...) {
...
## else (if the inverse has not been calculated)
